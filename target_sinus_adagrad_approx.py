...
accs = [theano.shared(grad.eval({x:x_train_set[0:batch_size].eval(),
                                y:y_train_set[0:batch_size].eval()})**2) for grad in grads]

updates = [(acc, acc+grad**2) for (acc, grad) in zip(accs, grads)]
updates.extend([(param, param-lr/(T.sqrt(epsilon+acc))*grad) for (param, lr, acc, grad) in zip(params, lrs, accs, grads)])
